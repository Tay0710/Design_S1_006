...
SparkFun_VL53L5CX myImagerU;
VL53L5CX_ResultsData measurementDataU; // Result data class structure, 1356 byes of RAM
...
void setup()
{
  ...
  I2C_bus1.begin(); // This resets I2C bus to 100kHz
  I2C_bus1.setClock(1000000); //Sensor has max I2C freq of 1MHz

  I2C_bus2.begin(SDAU,SCLU); // This resets I2C bus to 100kHz
...
void logToFU() {
  ...
  if(myImagerU.isDataReady() && myImagerU.getRangingData(&measurementDataU)) {
        // Serial.print("      read:");
        // now = micros();
        // Serial.printf("%.9f",now/1000000.0);
        // Serial.println("");
      ...
      for(int i = 0; i < imageResolution; i++) { //8x8 = 64; 4x4 = 16
          ...
          if (stat == 5){         
          idx += intToStr(measurementDataU.distance_mm[i], tofBuf + idx); // fast int -> string
          }
          else{
            tofBuf[idx++] = 'X';
          }
