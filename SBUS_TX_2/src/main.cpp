// https://blog.quadmeup.com/2017/10/25/generate-s-bus-with-arduino-in-a-simple-way/
...
void sbusPreparePacket(uint8_t packet[], int channels[], bool isSignalLoss, bool isFailsafe) {
  ...
  */
  for (uint8_t i = 0; i < SBUS_CHANNEL_NUMBER; i++) {
    ...
    packet[19] = (uint8_t) ((output[13] & 0x07FF)>>1);    packet[20] = (uint8_t) ((output[13] & 0x07FF)>>9 | (output[14] & 0x07FF)<<2);
  ...
  void loop() {
    ...
    */
    ...
    if (currentMillis > sbusTime) {
      ...
      Serial.print("send data at time: ");
      Serial.println(sbusTime);
    }
  }