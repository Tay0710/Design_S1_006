...
def animate_orientation(angle_history):
    data = np.array(angle_history)
    roll, pitch, yaw = np.radians(data[:, 1]), np.radians(data[:, 2]), np.radians(data[:, 3])
    ...
    # Keep handles to the quivers so we can update/remove them
    quivers = []
    ...
    def update(i):
        ...
        # Draw new arrows
        quivers.append(ax.quiver(0, 0, 0, *x_axis, color='r', label="X" if i == 0 else ""))
        quivers.append(ax.quiver(0, 0, 0, *y_axis, color='g', label="Y" if i == 0 else ""))
        quivers.append(ax.quiver(0, 0, 0, *z_axis, color='b', label="Z" if i == 0 else ""))
...
def main():  
    stateVector = np.array([0, 0, 0, 1, 0, 0, 0])  # [q0, q1, q2, q3, bx, by, bz]
    q_noise, b_noise = 1e-6, 1e-7
    Q = np.diag([q_noise, q_noise, q_noise, q_noise, b_noise, b_noise, b_noise])
    P = np.zeros((7,7))
    P[0:4, 0:4] = np.eye(4) * 1e-4
    P[4:7, 4:7] = np.eye(3) * 1e-3
    accelerometer_noise = np.diag([1e-3, 1e-3, 1e-3])

    previous_time = df.at[0, 'millis']
    currentTimeStep = 1
    dt = 0.0
    gyroscope_bias = calibrateGyroBias(df)
    ...
    animate_orientation(angle_history)
