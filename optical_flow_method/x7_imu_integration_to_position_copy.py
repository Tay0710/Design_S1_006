...
def process_data_2(filepath):
    ...
    return timestamp, delta_time, accelerometer
...
def process_data(filepath):
    ...
    accel_rej = 7 # Set max of the accel (7)
    mag_rej = 0
    rej_timeout = 2 * int(sample_rate)
    motion_threshold = 2 # 0.2
    ...
    for index in range(len(timestamp)):
        ...
        acceleration[index] = linear_accel_mps2 # was this broken???
        # acceleration[index] = 
        # print(earth_accel_g)
...
# === Function: Acceleration → Velocity ===
def accel_to_velocity_2(timestamp, acceleration, delta_time):
    # === Calculate velocity ===
    velocity = np.zeros((len(timestamp), 3))
    for index in range(len(timestamp)):

        velocity[index] = velocity[index - 1] + delta_time[index] * acc
...
# === Main ===
def main():
    input_csv = "../optical_flow_method_data/combined_samples/" + "17_09_25_MILC/2_L_shape_rot/" + "download_imu_cropped.csv"
    ...
    # acceleration = rotate_to_world(rot_mats, acceleration)
    ...
    plt.subplot(1, 3, 1)
    plt.plot(timestamp, acceleration[:, 0], label="Ax (m/s^2)")
    plt.plot(timestamp, acceleration[:, 1], label="Ay (m/s^2)")
    plt.plot(timestamp, acceleration[:, 2], label="Az (m/s^2)")
    plt.xlabel("Time (s)")
    ...
    plt.subplot(1, 3, 2)
    plt.plot(timestamp, velocity[:, 0], label="Vx (m/s)")
    plt.plot(timestamp, velocity[:, 1], label="Vy (m/s)")
    plt.plot(timestamp, velocity[:, 2], label="Vz (m/s)")
    plt.xlabel("Time (s)")
    ...
    plt.subplot(1, 3, 3)
    plt.plot(timestamp, position[:, 0], label="X (m)")
    plt.plot(timestamp, position[:, 1], label="Y (m)")
    plt.plot(timestamp, position[:, 2], label="Z (m)")
    plt.xlabel("Time (s)")
