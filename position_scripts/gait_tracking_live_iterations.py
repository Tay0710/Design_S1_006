...
# =========================================================
GRID_GAIN        = np.round(np.arange(1.2, 5, 0.2), 2)     # e.g. 0.6 .. 1.5
GRID_GYRO_RANGE  = [1500]                         # dps
GRID_ACCEL_REJ   = list(range(14, 19, 2))                    # 10,12,14,16,18
GRID_MAG_REJ     = [0]                                       # keep 0 unless you have mags
GRID_RT_FACT     = [1.0, 1.5, 2.0]                           # ×fs → samples
GRID_MT          = np.round(np.arange(0.45, 0.71, 0.05), 2)  # 0.45..0.70
GRID_SM_FACT     = [0.10, 0.12, 0.15, 0.18]                  # ×fs → samples

MAX_COMBOS       = 10000  # safety cap so you don't accidentally lock your machine
DEFAULT_SPEED_MS = 400   # default play speed (ms between steps)
